...
// An empty temp workspace. `injectGrokPwaHead` and `createHeadInjector` default
// `cwd` to `process.cwd()`, which in this app's repo is a real workspace with a
// committed `src/lib/og/site.json` and `public/og.jpg`. Tests that exercise the
// *placeholder* path must point at an empty dir so the bake/FS custom-card
// detection stays out — otherwise the repo's own card silently overrides the
// placeholder they are asserting on.
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: hermeticCwd() });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: hermeticCwd() });
  const chunks = [
