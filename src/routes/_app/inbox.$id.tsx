...
function Thread() {
  ...
  if (!data) {
    return <p className="text-muted">Conversation not found.</p>;
  }

  return (
    <div className="flex h-[calc(100dvh-8rem)] flex-col lg:h-[calc(100dvh-5rem)]">
      <div className="flex items-center gap-3 border-b border-border pb-3">
        <Link to="/inbox" className="grid size-10 place-items-center rounded-lg hover:bg-elevated lg:hidden">
        ...
        <Link to="/u/$handle" params={{ handle: data.other.handle }} className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-subtle">@{data.other.handle}</p>
          </div>
        </Link>
      </div>
      <div className="hide-scrollbar flex-1 space-y-2 overflow-y-auto py-4">
